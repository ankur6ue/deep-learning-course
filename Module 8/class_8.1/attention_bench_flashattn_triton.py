...
class AttentionInputs:
    ...
    sdpa_q: Optional[torch.Tensor] = None
    sdpa_k: Optional[torch.Tensor] = None
    sdpa_v: Optional[torch.Tensor] = None
    sdpa_attn_mask: Optional[torch.Tensor] = None
    sdpa_use_causal_flag: Optional[bool] = None
    flash_attn_q: Optional[torch.Tensor] = None
    flash_attn_k: Optional[torch.Tensor] = None
    flash_attn_v: Optional[torch.Tensor] = None
    flash_attn_k_cache: Optional[torch.Tensor] = None
    flash_attn_v_cache: Optional[torch.Tensor] = None
    flash_attn_cache_seqlens: Optional[torch.Tensor] = None
    flash_attn_block_table: Optional[torch.Tensor] = None
...
def prepare_vllm_triton_inputs(case: AttentionCase, inp: AttentionInputs) -> None:
    ...
    if case.paged_kv:
        ...
        '''
        In this block table, the page IDs are sequential. However, sequential page IDs are just a simplification from how 
        you lay out synthetic KV cache. In general paged KV cache, a single request’s KV blocks can be non-contiguous in memory.
        That’s what the block table encodes: for each sequence, it maps logical block/page position (0,1,2,...) to actual physical cache block IDs. So attention kernels gather K/V through this indirection instead of assuming contiguous pages.
        '''
        block_table = inp.page_table.to(dtype=torch.int32).contiguous()
        # tell the attention kernel about the length of each sequence
...
def prepare_sdpa_inputs(case: AttentionCase, inp: AttentionInputs) -> None:
    if inp.sdpa_q is not None:
        return
    ...
    inp.sdpa_k = k
    inp.sdpa_v = v
    inp.sdpa_attn_mask = attn_mask
    inp.sdpa_use_causal_flag = use_causal_flag


def prepare_flash_attn_inputs(case: AttentionCase, inp: AttentionInputs) -> None:
    if inp.flash_attn_q is not None:
        return

    inp.flash_attn_q = inp.q.permute(0, 2, 1, 3).contiguous()
    if case.is_decode:
        if case.paged_kv:
            if inp.k_pages is None or inp.v_pages is None or inp.page_table is None or inp.valid_lens is None:
                raise RuntimeError("paged KV inputs are incomplete")
            inp.flash_attn_k_cache = inp.k_pages.permute(0, 2, 1, 3).contiguous()
            inp.flash_attn_v_cache = inp.v_pages.permute(0, 2, 1, 3).contiguous()
            inp.flash_attn_cache_seqlens = inp.valid_lens.to(dtype=torch.int32)
            inp.flash_attn_block_table = inp.page_table.to(dtype=torch.int32)
        else:
            inp.flash_attn_k_cache = inp.k.permute(0, 2, 1, 3).contiguous()
            inp.flash_attn_v_cache = inp.v.permute(0, 2, 1, 3).contiguous()
        return

    inp.flash_attn_k = inp.k.permute(0, 2, 1, 3).contiguous()
    inp.flash_attn_v = inp.v.permute(0, 2, 1, 3).contiguous()
...
class AttentionBackend:
    ...
    def prepare(self, case: AttentionCase, inp: AttentionInputs) -> None:
        return None
...
class SDPABackend(AttentionBackend):
    ...
    def prepare(self, case: AttentionCase, inp: AttentionInputs) -> None:
        prepare_sdpa_inputs(case, inp)

    def run(self, case: AttentionCase, inp: AttentionInputs) -> torch.Tensor:
        assert inp.sdpa_q is not None
        assert inp.sdpa_k is not None
        assert inp.sdpa_v is not None
        assert inp.sdpa_use_causal_flag is not None
        ...
        try:
            ...
            with sdpa_kernel(backends=[SDPBackend.EFFICIENT_ATTENTION]):
                out = F.scaled_dot_product_attention(
                    ...
                    inp.sdpa_k,
                    inp.sdpa_v,
                    attn_mask=inp.sdpa_attn_mask,
                    dropout_p=0.0,
                    is_causal=inp.sdpa_use_causal_flag,
                )
        except Exception:
...
class FlashAttnBackend(AttentionBackend):
    ...
    def prepare(self, case: AttentionCase, inp: AttentionInputs) -> None:
        prepare_flash_attn_inputs(case, inp)

    def run(self, case: AttentionCase, inp: AttentionInputs) -> torch.Tensor:
        assert inp.flash_attn_q is not None
        if case.is_decode:
            if case.paged_kv:
                assert inp.flash_attn_k_cache is not None
                assert inp.flash_attn_v_cache is not None
                assert inp.flash_attn_cache_seqlens is not None
                assert inp.flash_attn_block_table is not None
                out = self.flash_attn_with_kvcache(
                    q=inp.flash_attn_q,
                    k_cache=inp.flash_attn_k_cache,
                    v_cache=inp.flash_attn_v_cache,
                    cache_seqlens=inp.flash_attn_cache_seqlens,
                    block_table=inp.flash_attn_block_table,
                    causal=case.causal,
                )
            else:
                assert inp.flash_attn_k_cache is not None
        ...
        assert inp.flash_attn_k is not None
        assert inp.flash_attn_v is not None
...
def benchmark_backend_case(
    ...
    try:
        ...
        max_abs_err = None
        mean_abs_err = None
        out = backend.normalize_output(case, inp, fn())
