...
def _attention_tail_masked_tensor_forward(
    ...
    The unmasked helper is kept for decode and all-valid prefill chunks because
    it avoids an extra mask argument and mask multiplies in the compiled graph.

    When prefill rows have different query lengths, the batch tensor is padded
    to `[batch, max_query_len, hidden]`. The padded rows do not represent real
    tokens, but the residual tensor is carried into the next layer. Masking only
    the final MLP output would leave padded residual rows nonzero, allowing them
    to drift through later layers. We therefore mask the attention branch before
    the residual add, and mask the MLP branch before returning.
    """
    attn_out = F.linear(
        attn_out_heads.reshape(-1, o_weight.shape[1]),
        o_weight,
