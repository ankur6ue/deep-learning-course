...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Choose the v4 attention path. 'dense_reference' preserves v3-style dense gather + assemble, while 'paged_sdpa' uses the new direct materialization path.",
    )
...
def main() -> None:
    ...
    print(
        summarize(
            f"simple_vllm_v4[{args.attention_backend}]",
            batched_run.wall_time_s,
            batched_lengths,
            batched_hits,
        )
    )
