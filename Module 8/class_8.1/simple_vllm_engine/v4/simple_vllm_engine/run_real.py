...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="paged_sdpa",
        choices=["dense_reference", "paged_sdpa"],
        help="Choose the v4 attention path. 'dense_reference' keeps the older dense gather + assemble reference path; 'paged_sdpa' uses the new direct paged materialization path.",
    )
    parser.add_argument(
        "--prompt",
        type=str,
...
def main() -> None:
    ...
    print(f"Running v4 batched engine ({engine_config.attention_backend})...")
    ...
    print(summarize(f"simple_vllm_v4[{engine_config.attention_backend}]", batched_wall, batched_results))
    if batched_wall > 0:
    ...
    if args.timing:
        ...
        print_timing_summary(f"simple_vllm_v4[{engine_config.attention_backend}]", batched_timings)
