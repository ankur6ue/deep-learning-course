...
def _prepare_decode_inputs_kernel(
    ...
    key_len = tl.where(active, past_len + 1, 1)
    ...
    scratch_slot = scratch_block_id * block_size
    ...
    tl.store(slot_mapping_ptr + row, tl.where(active, physical_slot, scratch_slot))
...
def prepare_decode_inputs(
    *,
    ...
    - `block_tables_i32`: int32 block table for device-side attention metadata
...
def describe_kernel_stack(device: str) -> str:
    ...
    if device.startswith("cuda"):
        return ('FlashAttention handles paged attention from block tables. v7 adds device-side decode metadata prep and GPU decode-state updates.')
