...
class PagedKVCache:
    ...
    def __init__(
        ...
        # Most Llama/Mistral layers share one KV-cache shape. Gemma 4 does not:
        # sliding layers use many 256-wide KV heads, while full-attention layers
        # use fewer 512-wide KV heads. Keep one tensor per layer so each layer's
        # cache exactly matches the K/V tensors produced by that layer.
        self.k_layers = []
        self.v_layers = []
        for layer_idx in range(model_config.num_layers):
            shape = (
                ...
                model_config.kv_heads_for_layer(layer_idx),
                model_config.head_dim_for_layer(layer_idx),
            )
            self.k_layers.append(torch.zeros(shape, device=device, dtype=dtype))
            self.v_layers.append(torch.zeros(shape, device=device, dtype=dtype))
