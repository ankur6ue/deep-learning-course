...
from .kernels import prepare_decode_inputs, prepare_decode_inputs_from_gpu_block_table
from .kernels import update_decode_state
...
def _cuda_graph_backend_supported(attention_backend: str) -> bool:
    return attention_backend in {"flash_attn_paged", "triton_paged"}
...
class DecodeGpuState:
    ...
    def __init__(self, *, max_num_reqs: int, device: torch.device, enabled: bool) -> None:
        ...
        # these values to gather/scatter rows in `last_token_ids` and gather
        # request block-table rows. Reusing one buffer avoids allocating a new
        # tiny CUDA tensor every decode step.
        ...
        # CUDA graph replay can make decode steps advance quickly. Before
        # `request_slots_for_batch()` refills `slot_cpu`, it waits for the
        # previous H2D copy to finish reading that pinned host buffer.
        # `slot_workspace` itself is reused on the main stream, so stream order
        # protects its GPU-side consumers.
        self.slot_copy_done_event = torch.cuda.Event() if self.enabled else None
        self.slot_copy_event_recorded = False
        # v9 adds a second GPU-resident request table:
        #
        #   request slot -> logical KV block -> physical KV block
        #
        # v8 rebuilt this table in Python for every decode batch. Here each
        # request row is updated only when `req.block_ids` grows. Decode steps
        # then pass a small list of request slots to a Triton kernel, which
        # gathers the active block-table rows on GPU.
        self.request_block_tables: torch.Tensor | None = None
        self.request_block_table_scratch_block_id: int | None = None
        self.request_id_to_synced_block_count: dict[str, int] = {}

    def enable_request_block_tables(self, *, max_blocks: int, scratch_block_id: int) -> None:
        """Allocate the persistent GPU block table used by v9 decode metadata.

        The table is keyed by stable request slot, not by transient decode batch
        row. Example with `block_size=16`:

            request_id_to_slot["req-A"] = 3
            req-A.block_ids             = [7, 12, 31]

        After synchronization, GPU row 3 contains:

            request_block_tables[3] = [7, 12, 31, scratch, scratch, ...]

        On a later decode step, the batch row only needs to say "use request
        slot 3"; the metadata kernel can fetch the block table directly on GPU.
        """
        if not self.enabled:
            return
        if max_blocks <= 0:
            raise ValueError("max_blocks must be positive")
        if self.request_block_tables is not None:
            if max_blocks <= self.request_block_tables.shape[1]:
                return
            old_table = self.request_block_tables
            new_table = torch.full(
                (old_table.shape[0], max_blocks),
                scratch_block_id,
                device=self.device,
                dtype=torch.long,
            )
            new_table[:, : old_table.shape[1]].copy_(old_table)
            self.request_block_tables = new_table
            self.request_block_table_scratch_block_id = scratch_block_id
            return
        self.request_block_tables = torch.full(
            (self.max_num_reqs, max_blocks),
            scratch_block_id,
            device=self.device,
            dtype=torch.long,
        )
    ...
    def allocate(self, request: RequestState) -> None:
        ...
        if self.enabled:
            ...
            if self.request_block_tables is not None:
                self.request_block_tables[slot].fill_(
                    int(self.request_block_table_scratch_block_id or 0)
                )
                self.request_id_to_synced_block_count[request.request_id] = 0
    ...
    def release(self, request: RequestState) -> None:
        ...
        if slot is not None:
            if self.enabled:
                ...
                self.request_id_to_synced_block_count.pop(request.request_id, None)
            self.free_slots.append(slot)

    def sync_request_blocks(self, request: RequestState) -> None:
        """Copy newly allocated block ids into the persistent GPU block table.

        This is intentionally called at KV allocation boundaries, not every
        decode step. If a request decodes 100 tokens with `block_size=16`, its
        block table grows only about seven times. v8 copied the whole visible
        block row on every one-token decode step.
        """
        if not self.enabled or self.request_block_tables is None:
            return
        self.allocate(request)
        slot = self.request_id_to_slot[request.request_id]
        block_count = len(request.block_ids)
        if block_count > self.request_block_tables.shape[1]:
            raise ValueError(
                "request needs more KV blocks than the persistent GPU block table supports"
            )
        synced = self.request_id_to_synced_block_count.get(request.request_id, 0)
        if block_count <= synced:
            return
        new_block_ids = torch.tensor(
            request.block_ids[synced:block_count],
            device=self.device,
            dtype=torch.long,
        )
        self.request_block_tables[slot, synced:block_count].copy_(new_block_ids)
        self.request_id_to_synced_block_count[request.request_id] = block_count

    def require_request_block_tables(self) -> torch.Tensor:
        """Return the persistent GPU block table or fail if it was not enabled."""
        if self.request_block_tables is None:
            raise RuntimeError("decode GPU request block table is not enabled")
...
class PrefillWorker:
    def __init__(
        ...
        self.prefill_graph_enabled = (
            ...
            and _cuda_graph_backend_supported(engine_config.attention_backend)
    ...
    def process(self, work_items: list[PrefillWorkItem]) -> None:
        ...
        with self.profiler.section("prefill.prepare"):
            ...
            for idx, item in enumerate(work_items):
                ...
                self.decode_gpu_state.sync_request_blocks(req)
...
class DecodeEagerWorkspace:
    ...
    def _resize_block_table_buffers(self, max_blocks: int) -> None:
        ...
        owns its logical-to-physical page map in `req.block_ids`, and v9 keeps a
        synchronized copy of that map in `DecodeGpuState.request_block_tables`.
        This workspace only owns the rectangular output tensors shaped:
        ...
        Resizing here only changes the storage available for that batch-shaped
        table. A Triton kernel fills it from the persistent GPU request table.
    ...
    def prepare(
        ...
        # Stage only the small per-row sequence lengths that change every decode step.
        # The full request block table is already resident on GPU and is updated
        # only when KV capacity grows. Request slots come from DecodeGpuState's
        # shared slot workspace below.
        self._wait_for_host_staging_reuse()
        for row_idx, (req, past_len, key_len) in enumerate(zip(requests, past_lengths, key_lengths, strict=True)):
            blocks_needed = self.kv_cache.blocks_needed(key_len)
            if blocks_needed > self.decode_gpu_state.require_request_block_tables().shape[1]:
                raise ValueError("request needs more block-table columns than the GPU request table has")
            self.cached_seq_lens_host[row_idx] = past_len

        # Move compact metadata to GPU once per decode step. Request slots are
        # already staged by DecodeGpuState; this workspace only owns sequence
        # lengths and the attention metadata outputs.
        ...
        prepare_decode_inputs_from_gpu_block_table(
            req_slots=req_slots,
            cached_seq_lens=self.cached_seq_lens[:batch_size],
            request_block_tables=self.decode_gpu_state.require_request_block_tables(),
...
class DecodeGraphBucket:
    ...
    def __init__(
        ...
        *,
        ...
        self.v_descale = torch.ones(descale_shape, device=device, dtype=torch.float32)
        pin_host = device.type == "cuda"
        # Graph buckets need their own fixed-address request-slot tensor. The
        # eager path reuses DecodeGpuState.slot_workspace, but a captured CUDA
        # graph cannot depend on a shared workspace whose address/lifetime is
        # owned by other eager helper calls.
    ...
    def _wait_for_host_staging_reuse(self) -> None:
        ...
        `_copy_batch()` writes request slots and cached lengths into pinned CPU
        tensors, then launches non-blocking H2D copies. CUDA
...
class DecodeWorker:
    def __init__(
        ...
        self.decode_metadata_max_blocks = self._max_decode_metadata_blocks()
        self.decode_metadata_scratch_block_id: int | None = None
        if (
            self.decode_gpu_state.enabled
            and (self.decode_graph_enabled or self.eager_decode_workspace_enabled)
        ):
            self.decode_metadata_scratch_block_id = self.kv_cache.allocate_block()
            self.decode_gpu_state.enable_request_block_tables(
                max_blocks=self.decode_metadata_max_blocks,
                scratch_block_id=self.decode_metadata_scratch_block_id,
            )
        if self.decode_graph_enabled:
            self.decode_graph_scratch_block_id = self.decode_metadata_scratch_block_id
        if self.eager_decode_workspace_enabled:
            if self.decode_metadata_scratch_block_id is None:
                raise RuntimeError("decode metadata scratch block was not allocated")
    ...
    def _max_decode_metadata_blocks(self) -> int:
        max_model_len = (
    ...
    def process(self, requests: list[RequestState]) -> None:
        ...
        with self.profiler.section("decode.prepare"):
            ...
            if batch_size > self.engine_config.max_decode_batch_size:
                raise ValueError("decode batch is larger than max_decode_batch_size")
            decode_req_slots: torch.Tensor | None = None
            for req in requests:
                ...
                try:
                    ...
                    if bucket.invalid:
                        ...
                        if (
                            ...
                            decode_req_slots = bucket.req_slots[:batch_size]
                except Exception as exc:
        ...
        with self.profiler.section("decode.postprocess"):
            ...
            if decode_req_slots is not None:
                self.decode_gpu_state.update_last_tokens_for_slots(
                    decode_req_slots,
                    next_tokens,
                    len(requests),
                )
...
class SimpleVLLMEngine:
    ...
    def warmup_decode_cuda_graphs(self, specs: list[RequestSpec]) -> DecodeGraphWarmupResult:
        ...
        request_block_tables = (
            self.decode_gpu_state.request_block_tables.clone()
            if self.decode_gpu_state.request_block_tables is not None
            else None
        )
        request_id_to_synced_block_count = dict(
            self.decode_gpu_state.request_id_to_synced_block_count
        )
        ...
        try:
            ...
            if request_block_tables is not None:
                self.decode_gpu_state.require_request_block_tables().copy_(request_block_tables)
            self.decode_gpu_state.request_id_to_synced_block_count = (
                request_id_to_synced_block_count
            )
