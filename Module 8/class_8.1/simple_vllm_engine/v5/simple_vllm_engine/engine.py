...
class SimpleVLLMEngine:
    ...
    def submit(self, spec: RequestSpec) -> RequestState:
        ...
        """
        requested_tokens = len(spec.prompt_ids) + spec.max_new_tokens
        max_positions = self.model_config.max_position_embeddings
        if requested_tokens > max_positions:
            raise ValueError(
                f"request {spec.request_id!r} asks for prompt_len + max_new_tokens "
                f"= {len(spec.prompt_ids)} + {spec.max_new_tokens} = {requested_tokens}, "
                f"but this v5 engine built a RoPE cache for only {max_positions} "
                "positions. Increase --max-model-len / max_position_embeddings, "
                "or reduce the prompt length or max_new_tokens."
            )
