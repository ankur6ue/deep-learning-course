...
if __name__ == "__main__":
    ...
    with mlflow.start_run():
        ...
        # Calculate accuracy on the entire dataset. We are treating the entire dataset as the test dataset..
        # Typically you'd split up the training dataset into non-overlapping train, validation and test subsets and
        # use k-fold cross validation.. but we are just keeping things simple for now
        logits = model(X)  # Forward pass
        loss = criterion(logits, Y)
        predicted_class = np.argmax(logits.detach().numpy(), axis=0)
        acc = np.mean(predicted_class == Y.detach().numpy())
        print(f"Accuracy on entire dataset = {acc: >4}")
        mlflow.log_metrics({"test_loss": loss.detach().numpy(), "test_accuracy": acc})
        metrics = {
            'hparam/accuracy': acc,
